...
in
(writeShellScriptBin "generic-update" ''
  set -euo pipefail
  ...
  if [ $HAS_CARGO -eq 1 ]; then
    JQ_ARGS+=(--arg cargo '${Utils.unreachableHash}')
  ...
  if [ $HAS_CARGO -eq 1 ]; then
    _LATEST_CARGO_HASH=$((nix build .#''${_LONEROS_NUR_KEY}.cargoDeps 2>&1 || true) | awk '/got/{print $2}')
  ...
  git commit --no-gpg-sign -m "''${_LONEROS_NUR_KEY}: ''${_LOCAL_VER:9} -> ''${_LATEST_VERSION:9}"
'').overrideAttrs
  (_prevAttrs: {
