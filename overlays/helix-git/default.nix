...
in
gitOverride (current: {
  lonerosNurKey = if evil then "evil-helix_git" else "helix_git";
  prev = prev.helix;

  versionLonerosNurPath =
    if evil then "overlays/helix-git/version-evil.json" else "overlays/helix-git/version.json";
  ...
  withExtraUpdateCommands = final.writeShellScript "bump-grammars" ''
    ...
    ${final.patch}/bin/patch -p1 -i "$_LONEROS_NUR_DIR/$_PKG_DIR/grammars.patch"
    ...
    ${final.jq}/bin/jq . < ./languages.json > "$_LONEROS_NUR_DIR/$_PKG_DIR/${languagesFile}"
    popd
